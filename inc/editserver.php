<?php
    ...
    $getid = "select id from users where username='".$_SESSION['user']."' LIMIT 1";
        if($user_row = $con->query($getid)){
            ...
            $editserver = "UPDATE `servers` SET `server_IP`='$server_IP', `server_Port`='$server_port', `server_status`='".pingServer($server_IP,$server_port)."',last_check='".date("d-m-y H:i:s")."', server_name='".$server_name."' WHERE server_ID='$server_ID' AND server_owner='".$user_row[0]."';"; // SQL to update the server
